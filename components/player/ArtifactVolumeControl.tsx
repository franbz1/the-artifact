...
interface ArtifactVolumeControlProps {
  ...
  /** Default: vertical dock (rotated). Use horizontal for popovers / mobile. */
...
export function ArtifactVolumeControl({
  ...
  const inputId =
    layout === "horizontal" ? "artifact-volume-inline" : "artifact-volume";
  ...
  const rangeInput = (
    <input
      ...
      className={cn(
        "control-strip-volume-range cursor-pointer rounded-sm outline-none focus-visible:ring-1 focus-visible:ring-lunar/50",
        layout === "vertical" ? "h-7" : "h-8 max-w-full",
        volumeTrackLit && "control-strip-volume-range--track-lit",
      )}
      style={
        layout === "vertical"
          ? { width: trackLengthPx }
          : { width: trackLengthPx, minWidth: 160 }
      }
    />
  );
  ...
  if (layout === "horizontal") {
    return (
      <div className={cn("flex w-full min-w-0 flex-col gap-1 py-1", className)}>
